...
import { setOrgAdmin } from "@/services/organization";
...
export function SetAdminModal({ isOpen, setIsOpen, currentMember, getMemberList }: { isOpen: boolean, setIsOpen: (value: boolean) => void, currentMember: any, getMemberList: () => void }) {
    ...
    const handleSetAdmin = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        setLoading(true);
        try {
            const res = await setOrgAdmin({ organization_id: currentMember?.id, user_ids: [currentMember?.id] });
            if (res.status_code === 200) {
                setIsOpen(false);
                message.success("Set organization admin success");
                getMemberList();
            } else {
                message.error(res.message || "Set organization admin failed");
            }
        } catch (error) {
            console.error(error);
        } finally {
