...
export default function OrganizationPage() {
    ...
    return <div className="flex flex-col px-4 py-4">
        ...
        <div className="flex flex-col gap-4 p-4">
            {organizationList.filter((organization) => organization?.is_active).map((organization) => (
                <OrganizationCard key={organization?.id} organization={organization} userType={userType} />
            ))}
            {
                organizationList.filter((organization) => !organization?.is_active).length > 0 && <>
                    <span className="text-lg text-gray-800 dark:text-gray-100 font-bold">Disabled Organizations</span>
                    {organizationList.filter((organization) => !organization?.is_active).map((organization) => (
                        <OrganizationCard key={organization?.id} organization={organization} userType={userType} />
                    ))}
                </>
