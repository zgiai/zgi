...
const MessageItem = ({ message, style }) => {
	...
	// Add a safe string conversion function
	...
	// Modify copy code functionality
	const handleCopyCode = (code: string, codeId: string) => {
		// If already showing "Copied" status, don't execute copy
				...
				<div className="flex items-center gap-2 p-3 bg-gray-50 rounded-lg">
					...
					{message.fileType && (
						<span className="text-xs text-gray-500">
							(
							{message.fileType.split("/").pop()?.toUpperCase() ||
								"Unknown format"}
							)
						</span>
					)}
	...
	return (
		<div style={style} className="py-2">
			<div
				className={`flex items-start gap-4 px-4 ${isUser ? "flex-row-reverse" : "flex-row"}`}
			>
