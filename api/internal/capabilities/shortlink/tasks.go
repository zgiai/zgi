...
const TypeShortLinkCleanup = "system:shortlink:cleanup"
...
func (h *CleanupExpiredHandler) Handle(ctx context.Context, task *asynq.Task) error {
	...
	deleted, err := h.service.CleanupExpired(ctx, time.Now(), 1000)
	if err != nil {
		logger.ErrorContext(ctx, "expired short link cleanup failed", err)
		return err
	}
	logger.InfoContext(ctx, "expired short link cleanup completed", "deleted_count", deleted)
	return nil
}
