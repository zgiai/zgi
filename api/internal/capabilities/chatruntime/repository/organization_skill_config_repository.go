...
func (r *organizationSkillConfigRepository) ListByOrganization(ctx context.Context, organizationID uuid.UUID) ([]*runtimemodel.OrganizationSkillConfig, error) {
	var configs []*runtimemodel.OrganizationSkillConfig
...
func (r *organizationSkillConfigRepository) ReplaceForOrganization(ctx context.Context, organizationID uuid.UUID, configs []*runtimemodel.OrganizationSkillConfig) error {
	now := time.Now()
	return r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		if err := tx.Where("organization_id = ?", organizationID.String()).Delete(&runtimemodel.OrganizationSkillConfig{}).Error; err != nil {
		...
		for _, config := range configs {
			...
			if err := tx.Exec(
				`INSERT INTO chat_runtime_organization_skill_configs (organization_id, skill_id, enabled, created_at, updated_at) VALUES (?, ?, ?, ?, ?)`,
...
func (r *organizationSkillConfigRepository) DeleteByOrganizationAndSkill(ctx context.Context, organizationID uuid.UUID, skillID string) error {
	...
	if err := r.db.WithContext(ctx).
		...
		Delete(&runtimemodel.OrganizationSkillConfig{}).Error; err != nil {
