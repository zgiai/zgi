...
var (
	analysisBlockPattern = regexp.MustCompile(`(?s)<analysis>.*?</analysis>`)
	summaryBlockPattern  = regexp.MustCompile(`(?s)<summary>(.*?)</summary>`)
	extraBlankPattern    = regexp.MustCompile(`\n\n+`)
)

func FormatCompactSummary(summary string) string {
	formatted := replaceFirstRegexp(summary, analysisBlockPattern, "")
	if match := summaryBlockPattern.FindStringSubmatchIndex(formatted); len(match) >= 4 {
		replacement := "Summary:\n" + strings.TrimSpace(formatted[match[2]:match[3]])
		formatted = formatted[:match[0]] + replacement + formatted[match[1]:]
	}
	formatted = extraBlankPattern.ReplaceAllString(formatted, "\n\n")
	return strings.TrimSpace(formatted)
}

func validCompactSummary(summary string) bool {
	match := summaryBlockPattern.FindStringSubmatch(summary)
	return len(match) >= 2 && strings.TrimSpace(match[1]) != ""
}

func replaceFirstRegexp(value string, pattern *regexp.Regexp, replacement string) string {
	match := pattern.FindStringIndex(value)
	if len(match) != 2 {
		return value
	}
	return value[:match[0]] + replacement + value[match[1]:]
}

func GetCompactUserSummaryMessage(summary string, suppressFollowUpQuestions bool, transcriptRef string, recentMessagesPreserved bool) string {
	...
	if strings.TrimSpace(transcriptRef) != "" {
		baseSummary += "\n\nIf you need specific details from before compaction (like exact code snippets, error messages, or content you generated), read the full transcript at: " + transcriptRef
	}
