...
func (s *service) beginClientActionContinuation(ctx context.Context, scope Scope, conversationID, messageID uuid.UUID, actionID string) (*ClientActionContinuation, error) {
	...
	if message.Status != runtimemodel.MessageStatusWaitingClientAction {
		if isResolvedClientActionEvent(event) {
			return nil, newContinuationAlreadyRunningError("client action continuation has already resolved; reconnect to the existing stream instead of retrying the action")
		}
...
func isResolvedClientActionEvent(event map[string]interface{}) bool {
	status := strings.ToLower(strings.TrimSpace(stringFromAny(event["status"])))
	if status == "" {
		return false
	}
	return status != clientActionStatusWaiting && status != runtimemodel.MessageStatusWaitingClientAction
}
...
func agentCreateTargetNamesFromText(text string) []string {
	...
	if names := agentCreateTargetNamesFromReadableText(text); len(names) > 0 {
		return names
	}
...
func agentCreateTargetNamesFromReadableText(text string) []string {
	text = strings.TrimSpace(text)
	if text == "" {
		return nil
	}
	lower := strings.ToLower(text)
	markers := []string{
		"名称分别为", "名字分别为", "名称为", "名字为", "命名为", "分别叫",
		"named ", "names are ",
	}
	start := -1
	markerLen := 0
	for _, marker := range markers {
		if index := strings.Index(lower, strings.ToLower(marker)); index >= 0 && (start < 0 || index < start) {
			start = index
			markerLen = len(marker)
		}
	}
	...
	stop := len(segment)
	for _, marker := range []string{
		"，描述", "，都写", "，不", "。不", "。完成", "完成后", "不要导航", "不要打开", "不要进入",
		", description", " with description", ", do not", ". do not", ". after",
	} {
		if index := strings.Index(strings.ToLower(segment), strings.ToLower(marker)); index >= 0 && index < stop {
			stop = index
		}
	...
	segment = strings.TrimSpace(segment[:stop])
	return splitAgentCreateTargetNamesReadable(segment)
}

func splitAgentCreateTargetNamesReadable(segment string) []string {
...
func agentCreateSharedDescriptionFromText(text string) string {
	...
	if description := agentCreateSharedDescriptionFromReadableText(text); description != "" {
		return description
	}
	lower := strings.ToLower(text)
...
func agentCreateSharedDescriptionFromReadableText(text string) string {
	text = strings.TrimSpace(text)
	if text == "" {
		return ""
	}
	lower := strings.ToLower(text)
	for _, marker := range []string{"描述都写为", "描述都写成", "描述写为", "描述写成", "描述为", "description is ", "description: "} {
		index := strings.Index(lower, strings.ToLower(marker))
		if index < 0 {
			continue
		}
		...
		for _, stop := range []string{"。", "，不要", "，不", "完成后", "不要导航", "不要打开", ". do not", ". after"} {
			if index := strings.Index(strings.ToLower(segment), strings.ToLower(stop)); index >= 0 {
				segment = segment[:index]
			}
		}
		return strings.Trim(segment, " \t\r\n\"'`“”‘’。")
