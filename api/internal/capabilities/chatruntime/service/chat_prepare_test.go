...
func (f fakeWaitingMessageRepo) GetScoped(ctx context.Context, id, organizationID, accountID uuid.UUID) (*runtimemodel.Message, error) {
	...
	return &runtimemodel.Message{ID: id, Status: status}, nil
}
...
func (f fakeWaitingMessageRepo) UpdateCompleted(context.Context, uuid.UUID, string, map[string]interface{}) error {
	panic("not implemented")
}
