...
func TestConsoleFilesActionDecisionIgnoresRuntimeContextOnlySpoof(t *testing.T) {
	...
	if isConsoleFilesContext(parts.RuntimeContext, parts.RawOperationContext, parts.OperationContext) {
		t.Fatalf("isConsoleFilesContext() = true, want false without structured console.files page resource")
	}
	if decision := consoleFilesActionDecisionForParts(parts); decision.Matched {
		t.Fatalf("Matched = true, want false without structured console.files page resource")
	}
}

func TestConsoleFilesCapabilityIgnoresUnstructuredDescriptionMention(t *testing.T) {
	operationContext := map[string]interface{}{
		"resources": []interface{}{
			map[string]interface{}{
				"resource_type": "page",
		...
		"capabilities": []interface{}{
			map[string]interface{}{
				"id":          "profile.read",
				"description": "This unrelated profile capability mentions file.read in prose.",
			},
		},
	}

	if !isConsoleFilesContext("", operationContext) {
		t.Fatal("isConsoleFilesContext() = false, want true for structured page resource")
	}
	if hasConsoleFilesReadCapability("", operationContext) {
		t.Fatal("hasConsoleFilesReadCapability() = true, want false when file.read appears only in description")
	}
}
