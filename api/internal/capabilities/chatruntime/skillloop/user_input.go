...
func normalizeUserInputRequestMessage(args map[string]interface{}) string {
	return trimRunes(stringFromInterface(args["message"]), maxUserInputMessageRunes)
}
...
func normalizeUserInputRequestArgs(args map[string]interface{}) ([]map[string]interface{}, error) {
	...
	for _, item := range raw {
		...
		question := trimRunes(stringFromInterface(typed["question"]), maxUserInputQuestionRunes)
		if question == "" {
			return nil, fmt.Errorf("%w: question is required", ErrInvalidInput)
		}
		normalized := map[string]interface{}{"question": question}
		if id := trimRunes(stringFromInterface(typed["id"]), 80); id != "" {
			normalized["id"] = id
		}
...
func normalizeUserInputOptions(value interface{}) ([]map[string]interface{}, error) {
	...
	for _, item := range raw {
		...
		label := trimRunes(stringFromInterface(typed["label"]), maxUserInputOptionLabelRunes)
		if label == "" {
		...
		if description := trimRunes(stringFromInterface(typed["description"]), maxUserInputOptionDescriptionRunes); description != "" {
...
func userInputQuestionSummaries(questions []map[string]interface{}) []interface{} {
	...
	for _, question := range questions {
		item := map[string]interface{}{
			"question": stringFromInterface(question["question"]),
		}
		if id := stringFromInterface(question["id"]); id != "" {
			item["id"] = id
		}
		if options, ok := question["options"].([]map[string]interface{}); ok && len(options) > 0 {
			labels := make([]interface{}, 0, len(options))
			for _, option := range options {
				if label := stringFromInterface(option["label"]); label != "" {
