...
func (s *Seeder) RunAll(ctx context.Context, force bool) error {
	...
	if !force {
		...
		if executed {
			if err := migrations.SeedBuiltInWorkflows(ctx, s.db); err != nil {
	...
	if err := s.db.QueryRowContext(
		...
		`SELECT COUNT(DISTINCT id)
		 ...
		 WHERE tenant_id::text = $1
		   AND id::text = ANY($2)`,
		migrations.BuiltInTenantID,
		pq.Array(migrations.BuiltInWorkflowSeedAgentIDs()),
	).Scan(&matchedCount); err != nil {
		return false, err
	}

	return matchedCount == len(migrations.BuiltInWorkflowSeedScenarios()), nil
}
