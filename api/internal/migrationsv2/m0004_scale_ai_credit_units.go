...
type legacySubscriptionPlanRow struct {
	ID          string            `gorm:"column:id"`
	QuotaConfig datatypes.JSONMap `gorm:"column:quota_config"`
}

func M0004_scale_ai_credit_units() *gormigrate.Migration {
	return &gormigrate.Migration{
		ID: migrationV2ScaleAICreditUnitsID,
		Migrate: func(tx *gorm.DB) error {
			if err := scaleZGIAPIBigintColumns(tx, "ai_credit_products", "credit_amount"); err != nil {
				return err
			}
...
func scaleZGIAPISubscriptionQuotas(tx *gorm.DB) error {
	...
	var plans []legacySubscriptionPlanRow
	...
	for _, plan := range plans {
		scaledConfig, changed, err := scaleLegacySubscriptionQuotaConfig(map[string]interface{}(plan.QuotaConfig))
		if err != nil {
