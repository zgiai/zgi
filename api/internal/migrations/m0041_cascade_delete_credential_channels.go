...
func M0041_cascade_delete_credential_channels() *gormigrate.Migration {
	return &gormigrate.Migration{
		...
		Migrate: func(tx *gorm.DB) error {
			...
			// Drop the existing RESTRICT foreign key constraint
			if err := tx.Exec(`
			...
			// When a credential is deleted, all channels using it will be automatically deleted
			return tx.Exec(`
				ALTER TABLE llm_system_channels
				ADD CONSTRAINT llm_system_channels_credential_id_fkey
				FOREIGN KEY (credential_id) REFERENCES llm_system_credentials(id) ON DELETE CASCADE
			`).Error
		},
