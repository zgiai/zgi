...
func TestEnsureBillingAttemptInvocationSourceMigration(t *testing.T) {
	db, mock := openMigrationMockDB(t)
	mock.ExpectQuery("SELECT EXISTS").WillReturnRows(sqlmock.NewRows([]string{"exists"}).AddRow(false))
	mock.ExpectExec("(?s).*ALTER TABLE.*billing_attempts.*invocation_source.*").WillReturnResult(sqlmock.NewResult(0, 0))

	builder := mschema.New(db)
	if err := upEnsureBillingAttemptInvocationSource(builder); err != nil {
		t.Fatal(err)
	}
