...
var BillingCommerceSchema = File{
    ...
    created_at timestamp with time zone DEFAULT CURRENT_TIMESTAMP NOT NULL
		...
		`COMMENT ON TABLE public.quota_usage_history IS 'Quota usage history';`,
		`COMMENT ON COLUMN public.quota_usage_history.group_id IS 'Organization ID';`,
		`COMMENT ON COLUMN public.quota_usage_history.account_id IS 'Operator account ID';`,
		`COMMENT ON COLUMN public.quota_usage_history.tenant_id IS 'Department ID, optional';`,
		`COMMENT ON COLUMN public.quota_usage_history.resource_type IS 'Resource type: seats, storage, db_rows, knowledge_bases, ai_agents, workflows, workflow_executions';`,
		`COMMENT ON COLUMN public.quota_usage_history.operation_type IS 'Operation type: increase or decrease';`,
		`COMMENT ON COLUMN public.quota_usage_history.delta IS 'Change amount; positive values increase usage and negative values decrease usage';`,
		`COMMENT ON COLUMN public.quota_usage_history.value_before IS 'Value before the change';`,
		`COMMENT ON COLUMN public.quota_usage_history.value_after IS 'Value after the change';`,
		`COMMENT ON COLUMN public.quota_usage_history.resource_id IS 'Related resource ID, such as a file or knowledge base ID';`,
		`COMMENT ON COLUMN public.quota_usage_history.resource_name IS 'Resource name';`,
		`COMMENT ON COLUMN public.quota_usage_history.metadata IS 'Detailed operation metadata in JSON format';`,
