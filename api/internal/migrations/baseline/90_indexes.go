...
var IndexesSchema = File{
	...
	Statements: []string{
		...
		`CREATE UNIQUE INDEX uk_roles_group_name ON public.roles USING btree (group_id, name) WHERE ((status)::text <> 'deleted'::text);`,
		`CREATE UNIQUE INDEX uk_roles_group_system_key ON public.roles USING btree (group_id, system_key) WHERE ((system_key IS NOT NULL) AND ((status)::text <> 'deleted'::text));`,
		...
		`CREATE INDEX idx_llm_invocation_contents_org_expires ON public.llm_invocation_contents USING btree (organization_id, expires_at);`,
		`CREATE INDEX idx_llm_invocation_contents_expires ON public.llm_invocation_contents USING btree (expires_at, request_id);`,
		`CREATE INDEX idx_llm_invocation_content_views_org_time ON public.llm_invocation_content_views USING btree (organization_id, viewed_at DESC);`,
		`CREATE INDEX idx_llm_invocation_content_views_request ON public.llm_invocation_content_views USING btree (request_id, viewed_at DESC);`,
