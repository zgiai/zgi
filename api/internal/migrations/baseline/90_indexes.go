...
var IndexesSchema = File{
	...
	Statements: []string{
		...
		`CREATE INDEX idx_agent_published_versions_agent_created ON public.agent_published_versions USING btree (agent_id, created_at DESC) WHERE (deleted_at IS NULL);`,
		`CREATE UNIQUE INDEX idx_agent_published_versions_version_uuid ON public.agent_published_versions USING btree (version_uuid) WHERE (deleted_at IS NULL);`,
		...
		`CREATE INDEX idx_chat_runtime_conversations_active_message ON public.chat_runtime_conversations USING btree (active_message_id) WHERE ((active_message_id IS NOT NULL) AND (deleted_at IS NULL));`,
		`CREATE INDEX idx_chat_runtime_conversations_owner_updated ON public.chat_runtime_conversations USING btree (organization_id, account_id, updated_at DESC) WHERE (deleted_at IS NULL);`,
		`CREATE INDEX idx_chat_runtime_conversations_caller_updated ON public.chat_runtime_conversations USING btree (organization_id, account_id, caller_type, caller_id, updated_at DESC) WHERE (deleted_at IS NULL);`,
		`CREATE INDEX idx_chat_runtime_conversations_runtime_status ON public.chat_runtime_conversations USING btree (runtime_status) WHERE (deleted_at IS NULL);`,
		...
		`CREATE INDEX idx_chat_runtime_messages_conversation_created ON public.chat_runtime_messages USING btree (conversation_id, created_at) WHERE (deleted_at IS NULL);`,
		`CREATE INDEX idx_chat_runtime_messages_parent ON public.chat_runtime_messages USING btree (parent_id) WHERE ((parent_id IS NOT NULL) AND (deleted_at IS NULL));`,
		`CREATE UNIQUE INDEX idx_chat_runtime_messages_source_message ON public.chat_runtime_messages USING btree (source_message_id) WHERE ((source_message_id IS NOT NULL) AND (deleted_at IS NULL));`,
		`CREATE INDEX idx_chat_runtime_organization_skill_configs_enabled ON public.chat_runtime_organization_skill_configs USING btree (organization_id, enabled);`,
