...
func validateSMSSubmitMethod(cfg SMSSubmitMethod) error {
	...
	if err := notificationsms.ValidateNotificationTemplateParams(template, map[string]string{
		notificationsms.TemplateParamNotificationTitle: cfg.NotificationTitle,
		notificationsms.TemplateParamLinkSuffix:        "/a/token",
	}); err != nil {
...
func approvalSMSTemplateParams(cfg SMSSubmitMethod, token string) map[string]string {
	...
	params[notificationsms.TemplateParamLinkSuffix] = approvalLinkSuffix(token)
	return params
}

func approvalLinkSuffix(token string) string {
