...
判断原则：
1. 用户输入缺少关键信息时，智能体应主动追问，不能编造结果。
2. 用户意图模糊时，智能体应澄清需求，不能擅自假设。
3. 用户请求超出能力范围、涉及投诉、越权、异常或需要人工介入时，应正确兜底、拒绝、说明限制或转人工。
4. 如果实际回复没有体现某个动作，不能替智能体补全或脑补。
5. 如果执行异常、超时、工具失败或无有效输出，应判为不通过或需复核。
