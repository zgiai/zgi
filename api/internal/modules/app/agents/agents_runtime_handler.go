...
func (h *AgentsHandler) validateAgentRuntimeSkills(c *gin.Context, skillIDs []string) error {
	...
	config, err := h.chatRuntimeService.GetSkillConfig(c.Request.Context(), runtimeservice.Scope{
		OrganizationID: organizationID,
		AccountID:      accountID,
	})
	if err != nil {
	...
	enabled := make(map[string]struct{}, len(config.EnabledSkillIDs))
	for _, id := range config.EnabledSkillIDs {
		enabled[strings.ToLower(strings.TrimSpace(id))] = struct{}{}
	}
	for _, raw := range skillIDs {
		...
		if _, ok := enabled[id]; !ok {
			return fmt.Errorf("skill %s is not enabled for this organization", id)
		}
	}
	return nil
}
