...
type ConversationRepository interface {
	Create(ctx context.Context, conversation *aichatmodel.Conversation) error
	GetScoped(ctx context.Context, id, organizationID, accountID uuid.UUID) (*aichatmodel.Conversation, error)
	GetBySourceConversation(ctx context.Context, sourceConversationID uuid.UUID) (*aichatmodel.Conversation, error)
	ListScoped(ctx context.Context, organizationID, accountID uuid.UUID, limit, offset int) ([]*aichatmodel.Conversation, int64, error)
...
type MessageRepository interface {
	Create(ctx context.Context, message *aichatmodel.Message) error
	GetScoped(ctx context.Context, id, organizationID, accountID uuid.UUID) (*aichatmodel.Message, error)
	GetBySourceMessage(ctx context.Context, sourceMessageID uuid.UUID) (*aichatmodel.Message, error)
	ListByConversationScoped(ctx context.Context, conversationID, organizationID, accountID uuid.UUID, limit, offset int) ([]*aichatmodel.Message, int64, error)
	ListBranch(ctx context.Context, leafID uuid.UUID, maxDepth int) ([]*aichatmodel.Message, error)
	CountByConversation(ctx context.Context, conversationID uuid.UUID) (int64, error)
	ReplaceRootForStreaming(ctx context.Context, message *aichatmodel.Message) error
...
type OrganizationSkillConfigRepository interface {
	ListByOrganization(ctx context.Context, organizationID uuid.UUID) ([]*aichatmodel.OrganizationSkillConfig, error)
	ReplaceForOrganization(ctx context.Context, organizationID uuid.UUID, configs []*aichatmodel.OrganizationSkillConfig) error
...
func (r *conversationRepository) Create(ctx context.Context, conversation *aichatmodel.Conversation) error {
	now := time.Now()
	if conversation.ID == uuid.Nil {
		conversation.ID = uuid.New()
	}
	if conversation.Status == "" {
		conversation.Status = aichatmodel.ConversationStatusNormal
	}
	if conversation.RuntimeStatus == "" {
		conversation.RuntimeStatus = aichatmodel.ConversationRuntimeStatusIdle
	}
	if conversation.Source == "" {
		conversation.Source = aichatmodel.ConversationSourceConsole
	}
...
func (r *conversationRepository) GetScoped(ctx context.Context, id, organizationID, accountID uuid.UUID) (*aichatmodel.Conversation, error) {
...
func (r *conversationRepository) GetBySourceConversation(ctx context.Context, sourceConversationID uuid.UUID) (*aichatmodel.Conversation, error) {
	var conversation aichatmodel.Conversation
	err := r.db.WithContext(ctx).
...
func (r *conversationRepository) ListScoped(ctx context.Context, organizationID, accountID uuid.UUID, limit, offset int) ([]*aichatmodel.Conversation, int64, error) {
	var conversations []*aichatmodel.Conversation
	var total int64
...
func (r *conversationRepository) StartStreaming(ctx context.Context, id, organizationID, accountID, messageID uuid.UUID) error {
	now := time.Now()
	result := r.db.WithContext(ctx).Model(&aichatmodel.Conversation{}).
		Where("id = ? AND organization_id = ? AND account_id = ? AND deleted_at IS NULL AND runtime_status <> ?", id, organizationID, accountID, aichatmodel.ConversationRuntimeStatusStreaming).
		Updates(map[string]interface{}{
			"runtime_status":    aichatmodel.ConversationRuntimeStatusStreaming,
...
func (r *conversationRepository) RefreshAfterMessageDelete(ctx context.Context, id uuid.UUID) error {
	var latest aichatmodel.Message
	...
	var count int64
	if err := r.db.WithContext(ctx).Model(&aichatmodel.Message{}).
		Where("conversation_id = ? AND deleted_at IS NULL", id).
...
func (r *messageRepository) Create(ctx context.Context, message *aichatmodel.Message) error {
	now := time.Now()
	if message.ID == uuid.Nil {
		message.ID = uuid.New()
	}
	if message.Status == "" {
		message.Status = aichatmodel.MessageStatusPending
	}
...
func (r *messageRepository) GetScoped(ctx context.Context, id, organizationID, accountID uuid.UUID) (*aichatmodel.Message, error) {
	var message aichatmodel.Message
	err := r.db.WithContext(ctx).Table("aichat_messages AS m").
		Select("m.*").
		Joins("JOIN aichat_conversations AS c ON c.id = m.conversation_id").
...
func (r *messageRepository) GetBySourceMessage(ctx context.Context, sourceMessageID uuid.UUID) (*aichatmodel.Message, error) {
	var message aichatmodel.Message
	err := r.db.WithContext(ctx).
...
func (r *messageRepository) ListByConversationScoped(ctx context.Context, conversationID, organizationID, accountID uuid.UUID, limit, offset int) ([]*aichatmodel.Message, int64, error) {
	var messages []*aichatmodel.Message
	var total int64
	query := r.db.WithContext(ctx).Table("aichat_messages AS m").
		Joins("JOIN aichat_conversations AS c ON c.id = m.conversation_id").
...
func (r *messageRepository) ListBranch(ctx context.Context, leafID uuid.UUID, maxDepth int) ([]*aichatmodel.Message, error) {
	if leafID == uuid.Nil || maxDepth <= 0 {
		return []*aichatmodel.Message{}, nil
	}

	out := make([]*aichatmodel.Message, 0, maxDepth)
...
func (r *messageRepository) ReplaceRootForStreaming(ctx context.Context, message *aichatmodel.Message) error {
	if message == nil {
	...
	result := r.db.WithContext(ctx).Model(&aichatmodel.Message{}).
...
func (r *messageRepository) UpdateStoppedAnswer(ctx context.Context, id uuid.UUID, answer string, metadata map[string]interface{}) error {
	...
	result := r.db.WithContext(ctx).Model(&aichatmodel.Message{}).
		Where("id = ? AND deleted_at IS NULL AND status IN ?", id, append(activeMessageStatuses(), aichatmodel.MessageStatusStopped)).
		Updates(map[string]interface{}{
			"answer":     answer,
			"status":     aichatmodel.MessageStatusStopped,
			"error":      nil,
...
func activeMessageStatuses() []string {
	return []string{aichatmodel.MessageStatusPending, aichatmodel.MessageStatusStreaming}
}
