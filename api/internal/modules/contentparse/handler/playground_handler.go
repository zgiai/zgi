...
func (h *PlaygroundHandler) RegisterRoutes(rg *gin.RouterGroup) {
	playground := rg.Group("")
	playground.Use(h.requirePlaygroundWorkspace())
	playground.GET("/playground/providers", h.ListProviders)
	playground.GET("/file-route/providers", h.ListFileRouteProviders)
	playground.POST("/playground/parse", h.Parse)
	playground.POST("/playground/save", h.SaveRun)
	playground.GET("/playground/admin/provider-summary", h.GetProviderSummary)
	playground.GET("/playground/compare/:hash", h.CompareRuns)
	playground.GET("/playground/share/:token/source-preview", h.RenderSharedRunSource)
	playground.GET("/playground/share/:token", h.GetSharedRun)
	playground.GET("/playground/runs", h.ListRuns)
	playground.POST("/playground/runs/:id/share", h.EnableRunShare)
	playground.GET("/playground/runs/:id/source-preview", h.RenderSavedRunSource)
	playground.GET("/playground/runs/:id", h.GetRun)
	playground.POST("/playground/pdf-render", h.RenderPDF)
}
