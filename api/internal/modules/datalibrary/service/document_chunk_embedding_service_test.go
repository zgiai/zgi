...
func TestDocumentChunkEmbeddingServiceCanIncludeDisabledChunks(t *testing.T) {
	assetID := uuid.New()
	runID := uuid.New()
	assetRepo := &fileAssetStateAssetRepo{
		asset: &model.DocumentAsset{
	...
	enabledChunk := &model.DocumentChunk{
		ID:              uuid.New(),
		OrganizationID:  "org-1",
		AssetID:         assetID,
		ProcessingRunID: runID,
		...
		ContentHash:     documentChunkContentHash("enabled child"),
		Enabled:         true,
		Status:          model.DocumentChunkStatusReady,
	}
	disabledChunk := &model.DocumentChunk{
		ID:              uuid.New(),
		OrganizationID:  "org-1",
		AssetID:         assetID,
		ProcessingRunID: runID,
		...
		ContentHash:     documentChunkContentHash("disabled child"),
		Enabled:         false,
		Status:          model.DocumentChunkStatusReady,
	}

	result, err := svc.GenerateAdditionalEmbeddings(context.Background(), GenerateDocumentChunkEmbeddingsInput{
		OrganizationID:        "org-1",
		AssetID:               assetID,
		ProcessingRunID:       runID,
		GenerationNo:          7,
		...
		Chunks:                []*model.DocumentChunk{enabledChunk, disabledChunk},
	})
	if err != nil {
		t.Fatalf("GenerateAdditionalEmbeddings: %v", err)
	}
	if result.EmbeddingCount != 2 || len(embeddingRepo.items) != 2 {
		t.Fatalf("result=%+v items=%d", result, len(embeddingRepo.items))
	}
	if !reflect.DeepEqual(embeddingSvc.texts, []string{"enabled child", "disabled child"}) {
		t.Fatalf("embedded texts=%+v", embeddingSvc.texts)
	}
}
