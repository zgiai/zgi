...
func skillToolArgumentContracts() map[string]SkillToolArgumentContract {
	return map[string]SkillToolArgumentContract{
		...
		SkillFileGenerator + "/generate_file": {
			...
			Description: "Generate a downloadable file artifact from provided content. Defaults to a temporary artifact; use target=managed_file only when the user explicitly asks to create it in File Management.",
			Schema: objectSchema(
				map[string]interface{}{
					...
					"lifecycle":    enumStringSchema("Temporary artifact lifecycle. Defaults to temporary and is ignored for target=managed_file.", []string{"persistent", "temporary"}),
					"target":       enumStringSchema("Generation target. Use temporary_artifact by default. Use managed_file only when the user explicitly asks to save/create/upload the result into File Management or the current files page.", []string{"temporary_artifact", "managed_file"}),
					"workspace_id": stringValueSchema("Optional workspace ID for target=managed_file. Usually omit so current workspace context is used. Do not invent IDs."),
					"folder_id":    stringValueSchema("Optional folder ID for target=managed_file when the user explicitly refers to a known folder. Do not invent IDs."),
				},
		...
		SkillFileGenerator + "/generate_docx": {
			...
			Description: "Generate a styled DOCX file from a structured JSON document specification. Defaults to a temporary artifact; use target=managed_file only when explicitly requested.",
			Schema: objectSchema(
				map[string]interface{}{
		...
		SkillFileGenerator + "/generate_pdf": {
			...
			Description: "Generate a styled PDF file from self-contained HTML and inline CSS. Defaults to a temporary artifact; use target=managed_file only when explicitly requested.",
			Schema: objectSchema(
				map[string]interface{}{
		...
		SkillFileGenerator + "/generate_pptx": {
			...
			Description: "Generate an editable static PPTX presentation from a structured JSON presentation specification. Defaults to a temporary artifact; use target=managed_file only when explicitly requested.",
			Schema: objectSchema(
				map[string]interface{}{
