...
when_to_use: Use this skill when the user asks to inspect, quote, summarize, compare, or answer from an uploaded file or a file shown in the console files page.
...
display:
  ...
  label:
    en_US: File Reader
  description:
    en_US: Reads accessible uploaded or console file text for the current AIChat turn.
  when_to_use:
...
Use this skill to read content from a file that has already been resolved for the current AIChat turn.
...
3. Call `read_file` with `file_id`. Set `max_chars` only when you need more or less returned content.
...
6. Mention the file name when summarizing or quoting content. Do not expose internal file IDs unless the user explicitly asks for them.
