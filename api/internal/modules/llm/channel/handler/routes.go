...
func RegisterTenantChannelRoutes(r *gin.RouterGroup, handler *ChannelHandler) {
	...
	admin.POST("/:id/upstream-state/check", handler.CheckUpstreamState)
	admin.POST("/:id/upstream-state/retry", handler.RetryUpstreamState)
	admin.PUT("/:id/upstream-state/settings", handler.UpdateUpstreamStateSettings)
	gAdmin := g.Group("")
