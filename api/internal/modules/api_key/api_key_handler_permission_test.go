...
func TestAPIKeyManagementRequiresAgentPermissionBeforeRequestHandling(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantPermission: workspace_model.WorkspacePermissionAgentView,
		},
		{
			name:           "get requires agent view before api key id validation",
			method:         http.MethodGet,
		...
		{
			name:           "usage logs require agent view before api key id validation",
			method:         http.MethodGet,
		...
		{
			name:           "usage stats require agent view before api key id validation",
			method:         http.MethodGet,
		...
		{
			name:           "create requires agent manage before body binding",
			method:         http.MethodPost,
			target:         "/agents/" + agentID.String() + "/api-keys",
			body:           `{"broken":`,
			params:         gin.Params{{Key: "agent_id", Value: agentID.String()}},
			call:           (*APIKeyHandler).CreateAPIKey,
			wantPermission: workspace_model.WorkspacePermissionAgentManage,
		},
		{
			name:           "update requires agent manage before body and api key id validation",
		...
		{
			name:           "delete requires agent manage before api key id validation",
		...
		{
			name:           "revoke requires agent manage before api key id validation",
			method:         http.MethodPost,
