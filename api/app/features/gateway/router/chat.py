    ...
    try:
        ...
        if request.stream:
            ...
            return StreamingResponse(
                stream_response(response_generator), media_type="text/event-stream"
            )
...
async def add_chat_messages(
    ...
    try:
        ...
        return resp_200(
            data=ConversationResponse(session_id=conversation.session_id),
            message="SUCCESS",
        )
    except Exception as e:
        error_response = {
...
async def get_conversations(
    ...
    try:
        ...
        count_query = db.query(Conversation).order_by(Conversation.created_at.desc())
        if user:
        ...
        for conversation in conversations:
            ...
            message_data = ChatMessages(session_id=conversation.session_id)
            if first_message:
...
async def get_chat_history(
    ...
    try:
        ...
        return resp_200(
            ChatMessagesResponse(messages=messages, session_id=session_id, total=total)
        )
    except ValueError as e:
        error_response = {
            "error": {
