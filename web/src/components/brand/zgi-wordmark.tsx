...
interface ZgiWordmarkProps {
  className?: string;
  title?: string;
}

export function ZgiWordmark({ className, title = 'ZGI' }: ZgiWordmarkProps) {
  return (
    <svg
      ...
      role="img"
      aria-label={title}
      className={cn('h-auto w-[116px]', className)}
    >
      <title>{title}</title>
      <path
      ...
      <path
        ...
        fill="#000"
      />
      <path d="m163.6 31.96h18.83v57.05h-18.83v-57.05z" fill="#000" />
      <path
