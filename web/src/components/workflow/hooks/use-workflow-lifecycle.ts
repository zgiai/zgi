...
const DEFAULT_CONVERSATIONAL_PROMPTS = {
  zh: {
    promptId: '9fa2ec04-0672-4e4f-9af6-ac4630f542ff',
    promptName: '企业助手回复模板',
    locale: 'zh-Hans',
    source: 'official' as const,
    text: '你是一名企业助手。请清晰回答问题，在必要时追问缺失上下文，并保持回复可执行、简洁、符合业务表达习惯。严格遵守政策边界。用户问题：{{#sys.query#}}',
  },
  en: {
    ...
    locale: 'en-US',
    source: 'official' as const,
    text: 'You are an enterprise assistant. Answer clearly, ask for missing context when needed, and keep the response actionable. Prefer concise business language and preserve policy boundaries. User question: {{#sys.query#}}',
  },
};

const DEFAULT_STANDARD_PROMPTS = {
  zh: {
...
export function useWorkflowLifecycle({
  ...
  const defaultConversationalPrompt = locale.toLowerCase().startsWith('zh')
    ? DEFAULT_CONVERSATIONAL_PROMPTS.zh
    : DEFAULT_CONVERSATIONAL_PROMPTS.en;
  const defaultStandardPrompt = locale.toLowerCase().startsWith('zh')
    ? DEFAULT_STANDARD_PROMPTS.zh
    : DEFAULT_STANDARD_PROMPTS.en;
