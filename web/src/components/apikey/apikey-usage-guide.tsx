...
export function ApiKeyUsageGuide({ apiBaseUrl = API_URL }: ApiKeyUsageGuideProps): JSX.Element {
  ...
  const usageOverviewMarkdown = React.useMemo(() => {
    return [
      ...
      `- ${t('usage.markdown.embeddingsLabel')}: \`${openAiBaseUrl}/embeddings\``,
      `- ${t('usage.markdown.imagesGenerationsLabel')}: \`${openAiBaseUrl}/images/generations\``,
  ...
  const curlExampleMarkdown = React.useMemo(() => {
    return [
      ...
      '```',
      '',
      `### ${t('usage.markdown.openAiEmbeddingsTitle')}`,
      '',
      '```bash',
      `curl --request POST '${openAiBaseUrl}/embeddings' \\`,
      "  --header 'Content-Type: application/json' \\",
      "  --header 'Authorization: Bearer YOUR_API_KEY' \\",
      "  --data '{",
      ...
      '```',
      '',
      `### ${t('usage.markdown.openAiImagesTitle')}`,
      '',
      '```bash',
      `curl --request POST '${openAiBaseUrl}/images/generations' \\`,
      "  --header 'Content-Type: application/json' \\",
      "  --header 'Authorization: Bearer YOUR_API_KEY' \\",
      "  --data '{",
