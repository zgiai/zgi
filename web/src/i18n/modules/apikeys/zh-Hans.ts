import type { ApikeysMessages } from './en-US';

const messages: ApikeysMessages = {
  title: 'API 密钥',
  description: '管理访问 LLM 服务的 API 密钥',
  listTitle: 'API 密钥列表',
  searchPlaceholder: '搜索名称',
  filters: {
    allStatus: '全部状态',
    active: '激活',
    inactive: '未激活',
    revoked: '已撤销',
    startDate: '开始日期',
    endDate: '结束日期',
  },
  status: {
    active: '激活',
    inactive: '未激活',
    revoked: '已撤销',
  },
  table: {
    name: '名称',
    key: 'API 密钥',
    status: '状态',
    usedQuota: '已用额度',
    remainQuota: '剩余额度',
    modelLimits: '模型限制',
    createdAt: '创建时间',
    updatedAt: '更新时间',
    expiresAt: '过期时间',
    noExpiration: '永不过期',
    enabled: '启用',
    actions: '操作',
  },
  actions: {
    add: '创建 API 密钥',
    edit: '编辑',
    delete: '删除',
    copy: '复制密钥',
    copied: '已复制！',
    batch: '批量操作',
    confirmDeleteTitle: '删除 API 密钥',
    confirmDeleteDesc: '确定要删除此 API 密钥吗？此操作无法撤销。',
    cancel: '取消',
    confirm: '确认',
  },
  usage: {
    badge: 'OpenAI 兼容',
    inlineTitle: '创建后的 API Key 可直接用于平台接口调用',
    inlineDescription:
      'apikey 创建后可通过本平台的接口调用，支持通用 OpenAI 协议。当前支持基础对话调用。',
    apiBaseLabel: 'API Base：',
    openDialog: '查看接入说明',
    dialogTitle: 'API 调用说明',
    dialogDescription: '查看基础接入信息，并在 curl、TypeScript、Python 示例之间切换。',
    markdown: {
      overviewTitle: '基础接入信息',
      overviewBody: 'apikey 创建后可通过本平台的接口调用，支持通用 OpenAI 协议。',
      apiBaseLabel: 'API Base',
      openAiBaseLabel: 'OpenAI Base URL',
      chatCompletionsLabel: 'Chat Completions 接口',
      supportNote: '当前支持：基础对话调用。',
      tabs: {
        curl: 'curl',
        ts: 'TypeScript',
        python: 'Python',
      },
      exampleTitle: 'curl 调用示例',
      exampleNote: '请将 `YOUR_API_KEY` 替换为你的密钥，并将 `your-model-name` 替换为可用模型名。',
      tsExampleTitle: 'OpenAI SDK 调用示例（Node.js / TypeScript）',
      tsInstallLabel: '安装',
      tsExampleNote:
        '请在服务端使用该示例，并通过 `PLATFORM_API_KEY` 之类的环境变量传入密钥，同时将 `your-model-name` 替换为可用模型名。',
      pythonExampleTitle: 'OpenAI SDK 调用示例（Python）',
      pythonInstallLabel: '安装',
      pythonExampleNote:
        '请在服务端使用该示例，并通过 `PLATFORM_API_KEY` 之类的环境变量传入密钥，同时将 `your-model-name` 替换为可用模型名。',
    },
  },
  dialog: {
    titleCreate: '创建 API 密钥',
    titleEdit: '编辑 API 密钥',
    basic: '基本信息',
    advanced: '高级设置',
    labels: {
      name: '名称',
      count: '数量',
      quotaType: '额度类型',
      quotaAmount: '额度数量',
      allowAllModels: '允许所有模型',
      modelNames: '允许的模型',
      allowIps: 'IP 白名单',
      expiresAt: '过期时间',
      status: '状态',
      modelLimitsEnabled: '启用模型限制',
    },
    placeholders: {
      name: 'API 密钥名称',
      count: '1',
      quotaAmount: '1000',
      allowIps: '192.168.1.1, 10.0.0.0/8',
    },
    quotaTypes: {
      unlimited: '无限制',
      custom: '自定义',
    },
    buttons: {
      cancel: '取消',
      create: '创建',
      save: '保存',
    },
    createdTitle: 'API 密钥创建成功！',
    createdNotice: '请立即复制并妥善保存此密钥，它将不会再次显示。',
    errors: {
      invalidIp: 'IP 地址格式无效',
      duplicateIp: 'IP 地址已存在',
      expiresInPast: '过期时间必须是未来时间',
      quotaAmountRequired: '额度数量为必填项，且必须为非负整数',
      quotaAmountMax: '额度数量不能超过 {max}',
    },
    hints: {
      expiresAt: '留空表示永不过期',
      allowAllModelsEnabled: '此 API 密钥允许访问所有模型',
      quotaAmountMax: '最大限制：{max}',
    },
  },
  empty: '暂无 API 密钥',
  updateSuccess: 'API 密钥更新成功',
  deleteSuccess: 'API 密钥删除成功',
  createSuccess: 'API 密钥创建成功',
};

export default messages;
