...
- The current implementation deliberately keeps webapp/API audience grants public-only. Moving to user/department-targeted webapp/API access still requires the recorded product decision on public config/auth handshake, webapp migration ownership proof, and API key caller/owner semantics.
