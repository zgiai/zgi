...
The guard rejects personal calls on old binaries; it does not make those binaries support the feature or preserve its availability. The migration can also cause earlier grant-aware binaries to reject personal calls until the updated authentication paths are deployed. Coordinate the migration and API switch in a controlled release window. Do not describe a model-level or isolated database test as proof of a safe mixed-version deployment.
