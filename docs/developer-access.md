...
Personal keys must not be served by API binaries that predate principal and grant validation. The keys share the legacy key table and hash lookup. New and rotated personal keys store an independent `quota_limit = 0` and `remain_quota = 0` as a fail-closed compatibility guard; only the grant-aware authentication paths use their live grant allowance. Legacy organization keys retain their existing quota behavior.

Migration `20260907150000_guard_personal_key_legacy_quota` backfills those two guard fields on existing principal-bound keys and adds a database constraint preventing null or positive independent quotas. It does not change grant balances, usage counters, or billing attribution. Earlier personal keys used a null independent quota, which old binaries interpret as unlimited. Previously cached copies can retain that value for the API-key cache TTL (currently five minutes). Before admitting any old binary, stop personal-key traffic and old cache writers, complete the migration, then invalidate the affected entries or wait longer than the full TTL and verify rejection on both cache misses and hits. Database backfill alone is not sufficient.

The guard rejects personal calls on old binaries; it does not make those binaries support the feature or preserve its availability. The migration can also cause earlier grant-aware binaries to reject personal calls until the updated authentication paths are deployed. Use a two-stage release: first deploy and verify the compatible authentication and key-creation code without this backfill, accepting existing null and new zero independent quotas only after live Grant validation. Retain that verified artifact as the rollback target. Only then ship the migration and constraint, handling pre-migration cache lifetime as described above. Do not describe a model-level or isolated database test as proof of a safe mixed-version deployment.
