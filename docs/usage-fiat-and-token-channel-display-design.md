...
![用量总览中的法币费用卡片](./images/usage-fiat-summary-dev2.jpg)

上图展示费用卡片的最终形态：调用数和 Token 保留原始统计单位；官方渠道、私有渠道和总消耗统一展示为组织配置的法币。截图中的 `≈¥` 表示当前组织使用人民币展示，并按组织汇率从 USD 换算。
...
![每日 Token 趋势中的官方与私有渠道堆叠柱](./images/usage-token-channel-trend-dev2.jpg)

上图中每根柱代表一天的总 Token：蓝色部分是官方渠道，橙色部分是私有渠道。两部分垂直堆叠，因此既能比较每日总量，也能直接观察渠道组成。
...
- 文档截图来自 dev2 验证数据，只用于说明页面结构和视觉编码，不代表生产用量。
