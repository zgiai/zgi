...
> 当前实现说明（2026-08-17）：运行时不再把 Agent 上下文 checkpoint 写入本地文件，也不从本地文件恢复。`ContextManager` 只维护当前进程内的上下文状态；跨进程断点恢复将在后续改为基于数据库中的 Agent 轨迹实现。本文后续关于本地 checkpoint 的描述保留为历史设计背景，不代表当前运行时行为。
