...
<p align="center">
  <sub>
    ...
    <a href="#プロダクトツアー">プロダクトツアー</a> &middot;
...
管理対象のデータテーブルを選択し、自然言語の質問から追跡可能なKPI、リスク、推奨アクションを生成します。

![ZGIで契約更新リスクを自然言語分析](docs/assets/product-tour/zgi-data-analysis-en.webp)

### 実行結果を確認

実行状態、所要時間、各ステップ、構造化された出力を確認し、ワークフローで実行された処理と返された結果を追跡します。

![ZGIワークフローの実行結果と構造化されたリスク情報](docs/assets/product-tour/zgi-run-results-en.webp)

### モデルとチャネルを一元管理

プロバイダー、チャネル、ルーティングポリシー、可用性を一元的に管理します。

![ZGI Model Gatewayのプロバイダーとチャネル管理](docs/assets/product-tour/zgi-model-gateway-en.webp)

### 社内ファイルをエージェントの情報源として活用

ファイルをアップロードして処理し、承認済みのナレッジベースに接続して、エージェントとワークフローから利用します。

![ZGIのファイル管理とナレッジベース連携](docs/assets/product-tour/zgi-files-knowledge-en.webp)
