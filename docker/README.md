...
When started from `zgi-pre/docker`, `sandbox` reuses the shared root Postgres and Redis services:
...
That applies recommended build mirrors for the current run while keeping service runtime env files in their own submodules.
...
- `api/`, `web/`, `sandbox/`, and `plugin-runner/` keep their own runtime env files inside each submodule directory.
- `dev/bootstrap` initializes submodules, copies missing env templates into each submodule, and regenerates `docker-compose.yaml`.
...
- `./dev/start-docker --china` currently wires China mainland build mirrors for `api`, `sandbox`, and `plugin-runner` through compose build args.
